...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        tasks = Task.objects.filter(
            Q(deadline__isnull=True) | Q(deadline__gte=min_deadline), closed=False, archived=False
        )
        for task in tasks:
        ...
        events = ProgressEvent.objects.filter(
            task__archived=False, task__closed=False,
            due_at__range=[right_now, future_by_8_hours], last_reminder_at__isnull=True
        )
        for event in events:
